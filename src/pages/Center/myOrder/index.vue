          ...
          <tr v-for="(cart,index) in order.orderDetailList" :key="cart.id">
            <td style="width: 60%">
              <div class="typographic">
                <img style="height: 100px;width: 100px" v-lazy="cart.imgUrl">
            ...
            <template v-if="index === 0">
              <td :rowspan="order.orderDetailList.length" style="width:8%" class="center">{{ order.consignee }}</td>
              <td :rowspan="order.orderDetailList.length" style="width:13%" class="center">
                <ul class="unstyled">
                  <li>总金额 ¥{{ order.totalAmount }}</li>
                  <li>在线支付</li>
                </ul>
              </td>
              <td :rowspan="order.orderDetailList.length" style="width:8%" class="center">
...
const getData = (page,limit)=>{
  store.dispatch('order/getOrderList', {page, limit});
}

//获取订单列表
const orderListInfo = computed(() => {
  return store.getters["order/orderList"];
})

// 订单商品列表
const orderList = computed(() => {
  return orderListInfo.value.records || [];
})
