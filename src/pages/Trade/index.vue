  ...
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="address clearFix" v-for="address in addressList" :key="address.id"
           @click="changeDefault(addressList,address)">
      ...
      <div class="detail">
        <h5>商品清单</h5>
        <ul class="list clearFix" v-for="total in order.detailArrayList" :key="total.skuId">
          <li>
            <img style="height: 100px;width: 100px" :src="total.imgUrl" alt="">
          </li>
          <li>
            <p>{{ total.skuName }}</p>
            <h4>7天无理由退货</h4>
          ...
          <li>
            <h3>￥{{ total.orderPrice }}</h3>
          </li>
          <li>X{{ total.skuNum }}</li>
          <li>有货</li>
        </ul>
      </div>
      <div class="bbs">
    ...
    <div class="trade">
      <div class="price">应付金额:　<span>¥{{ order.originalTotalAmount }}</span></div>
...
// 地址信息
const addressList = computed(() => store.getters["trade/addressList"]);

// 修改默认地址
const changeDefault = (addressList, address) => {
...
// 订单信息
const order = computed(() => store.getters["trade/order"])
...
const toPay = async () => {
  try {
    let orderId = await store.dispatch('trade/submitOrder', {
      tradeNo: order.value.tradeNo,
      data: {
        consignee: defaultAddress.value.consignee,
        consigneeTel: defaultAddress.value.phoneNum,
        deliveryAddress: defaultAddress.value.userAddress,
        paymentWay: 'ONLINE',
        orderComment: msg.value,
        orderDetailList: order.value.detailArrayList
      }
    });
    await router.push({
      name: 'pay', query: {
