          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="spuSaleAttr in spuSaleAttrList"
                ...
                <dd changepirce="0"
                    v-for="(SaleAttrValue,index) in spuSaleAttr.spuSaleAttrValueList"
                    :key="SaleAttrValue.id"
                    :class="{active:SaleAttrValue.isChecked === '1'}"
                    @click="changActive(spuSaleAttr.spuSaleAttrValueList,index)">
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <a @click="addCartSuccess">加入购物车</a>
              </div>
            </div>
          </div>
        </div>
...
const getData = () => {
  // 更新vuex仓库内商品详情数据
  store.dispatch('detail/getGood', route.query.skuId);
}
...
const changeSkuNum = (e) => {
  ...
  if (isNaN(num * 1) || num < 1) {
    ...
    skuNum.value = parseInt(num);
  }
}

// 选择商品参数的业务逻辑
const changActive = (list, index) => {
  list.forEach((item) => {
    item.isChecked = '0';
  })
...
const addCartSuccess = async () => {
  try {
    await store.dispatch('detail/addOrUpdateCartSuccess', {skuId: route.query.skuId, skuNum: skuNum.value})
    await router.push({
      name: 'addCartSuccess',
      query: {
        skuNum: skuNum.value,
